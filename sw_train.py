    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if sw_train:                
                ...
                len_train_tmp = len(sel_train_tmp)
                for index,(_coder_channels,_en_stride) in enumerate(sel_train_tmp):
                    ema.ema.apply(lambda m: setattr(m, 'coder_channels', _coder_channels))
                    ema.ema.apply(lambda m: setattr(m, 'en_stride', _en_stride))
                    # print(model.coder_channels)
                    # print(model.en_stride)
                    # with open('model1207.txt','w') as f:
                    #     f.write(str(model))
                    # mAP
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='yolov5s-wfz.yaml', help='model.yaml path')
